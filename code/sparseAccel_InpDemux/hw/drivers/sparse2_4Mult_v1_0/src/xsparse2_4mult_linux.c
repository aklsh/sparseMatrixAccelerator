...
typedef struct {
    ...
    XSparse2_4mult_uio_map maps[ MAX_UIO_MAPS ];
} XSparse2_4mult_uio_info;

/***************** Variable Definitions **************************************/
static XSparse2_4mult_uio_info uio_info;
...
static int uio_info_read_name(XSparse2_4mult_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSparse2_4mult_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSparse2_4mult_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSparse2_4mult_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSparse2_4mult_Initialize(XSparse2_4mult *InstancePtr, const char* InstanceName) {
	XSparse2_4mult_uio_info *InfoPtr = &uio_info;
